...
for i=1:nSamples,
    ...
    if numel(tdpdir)==0 || data_date>tdpdir.datenum || force_remake_tdplot
    ...
    if no_statehist, continue; end
    
    shdir = dir(shist_fname);
    
    if numel(shdir)==0 || data_date>shdir.datenum || force_remake_tdplot
...
for i=1:numel(samples),  %for each sample
    ...
    elseif ~exist(hist_filename,'file') || fileIsNewer(data_fname,hist_filename)
        ...
        cplotdata = makecplot( filename, contour_bin_size );
    else
...
%          set(gca,'ytick',0:0.2:1.0);
    ...
    if ~exist(shist_fname,'file') || no_statehist
        ...
        pophist = sum(histdata,2)/N(i)/pophist_sumlen;   %normalization
