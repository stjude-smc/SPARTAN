function varargout = sorttraces_rtd(varargin)
...
%    traceStat (which requires: RLE_filter, CalcLifetime)
...
if 1, %...
    ...
    sig = ccdFilter(donor,acceptor,15);
    v = zeros(1,handles.len);
    v( sig<0 ) = 20000;

    grey = [0.95 0.95 1];
    bar( 1:handles.len, v, 1, 'FaceColor',grey, 'EdgeColor',grey );
...
hold off;
cla;
% plot( time,signal,'k', time,handles.fretThreshold*ones(1,handles.len),'m');
% dd = gradient( medianfilter(donor,3) ) / stats.t;
% da = gradient( medianfilter(acceptor,3) ) / stats.t;
dd = gradient( donor ) / stats.t;
da = gradient( acceptor ) / stats.t;
et = - (dd.*da) *5;
et(et<0)=0;

% plot( time, dd,'g', time,da,'r', time,et,'k' );
plot( time,et,'k' );

% plot(time, ccdFilter(donor,acceptor,5) );
...
% Add a line at 0.125 and bars highlighting regions above this threshold.
% Useful for detecting events for molecular association (RTD)
plot(1:handles.len,repmat(0.125,handles.len,1),'m-'); 
hold on;

if 1,
    inds = find( fret >= 0.125 );
    v = zeros( 1,handles.len );
    v(inds) = 1.0;
