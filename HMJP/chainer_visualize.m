...
if isempty(Gim)
    ...
    mti = max(chain.params.Int_D(chain.sample.t_s)+chain.params.Int_A(chain.sample.t_s));
    line(chain.params.T_i.*[1 1],[0,mti],'linestyle','--','color','k','linewidth',2);
    line(chain.params.T_f.*[1 1],[0,mti],'linestyle','--','color','k','linewidth',2);
    ...
    if toc(timer)<0.5, return; end   %limit frequency of updates for speed.
