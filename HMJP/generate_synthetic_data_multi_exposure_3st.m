function [observation,units,ground] = generate_synthetic_data_multi_exposure_3st(tag,show_demo)
...
obs_wind_left  = (0.1 :dt_1 :6)'; %same size as obs(COLUMN)
...
mu_D            =  [800;3000;6000]; % photons/sec  mean state(1 2 3) = (1 2 5)(units of space)
mu_A            =  [300;2000;5000]; % photons/sec  mean state(1 2 3) = (1 2 5)(units of space)
...
M             =  zeros(size(mu_D,1),size(mu_D,1) );
...
save(['simulated_data_' num2str(tag),'_alpha_',num2str(alpha),'.mat'],'ground','observation','units','-v7.3')
